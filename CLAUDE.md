...
- `review-loop-guide.md` - Tick-driven loop guide for /review-doc-loop and /exam-loop (single source of truth for loop mechanics)
...
- `/review-doc-loop` - Tick-driven loop that coordinates with `/exam-loop` via the shared review doc (long-running, main conversation only)
